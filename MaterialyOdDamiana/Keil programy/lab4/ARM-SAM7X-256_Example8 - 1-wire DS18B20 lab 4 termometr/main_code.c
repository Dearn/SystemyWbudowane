...
int main(void) {
  ...
  //LCDPutStr("Hello world", 15, 15, LARGE, PINK, BLACK);
	...
	while(1) 
	{
		...
		sprintf(ciag,"Tw=%3.2f",(temp*10/0.305)+20);
		LCDPutStr(ciag,30,10,MEDIUM,BLACK,MAGENTA);
		
		//Temperatura zewnetrznego czujnika
		if(owReadRom(rom)==OK)
		{ //tu gdy odczyt przegiegl pomyslnie i funkcja zwrocila wartosc "OK"
			ds18b20ConvertT();
			delay_ms(1000);
			temperatura_ADC= ds18b20ReadTempNoRom();
			temperatura=temperatura_ADC*0.0625; //1/16
			
			sprintf(ciag, "Tz=%3.1f  ", temperatura);
			LCDPutStr(ciag, 40, 10, MEDIUM, BLACK, YELLOW);
		} else //tutaj odczyt zakoDczony niepowodzeniem
		{
			sprintf(ciag, "Tz=??     ");
			LCDPutStr(ciag, 40, 10, MEDIUM, BLACK, YELLOW);
		}
