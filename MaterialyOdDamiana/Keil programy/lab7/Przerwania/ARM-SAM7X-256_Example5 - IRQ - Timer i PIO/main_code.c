...
static volatile int zmienna_1, zmienna_2,s,m,h;
...
__irq void pioIsr2(void) 
{
	...
	if (status_IRQ & (1<<24)) //jesli PA7 wywolalo przerwanie... (dowolna zmiana stanu na linii PA7 = wcisniecie lub puszczenie klawisza)
		{
			//kod dla zmiany PA7
			if (LEFT_STICK) //dodatkowo sprawdzamy, czy aktualny stan to wcisniety klawisz? (czyli zdarzenie, które wystapilo to wcisniecie)
			{
		...
		if (status_IRQ & (1<<25)) //jesli PA14 wywolalo przerwanie... (dowolna zmiana stanu na linii PA8 = wcisniecie lub puszczenie klawisza)
		{
			//kod dla PA8...
			zmienna_1++;
		}
...
const unsigned char PIO_IRQ_PRIORITY = 5;
  ...
  // Init LCD
   ...
   LCDClearScreen();
	...
	AT91C_BASE_PIOB->PIO_IER = (1<<24) | (1<<25); //przerwania beda generowane w reakcji na zmiane stanu linii PA7 i PA14
	//PA7 = left, PA8 = down, PA9 = UP, PA14 = PRAWO, AP15 = CLICK
  ...
  // draw a string
  LCDPutStr("Interrupts", 15, 15, SMALL, PINK, BLACK);
	...
	while(1) 
	{
		...
		sprintf(ciag, "zmiena1=%d  ", zmienna_1);
		LCDPutStr(ciag, 30, 10, SMALL, BLACK, YELLOW);

		sprintf(ciag, "zmiena2=%1.1f  ", zmienna_2*0.01);
		LCDPutStr(ciag, 40, 10, SMALL, BLACK, YELLOW);
		
		sprintf(ciag, ":%1.1d  ", s);
		LCDPutStr(ciag, 60, 60, SMALL, BLACK, GREEN);
		sprintf(ciag, ":%1.1d  ", m);
		LCDPutStr(ciag, 60, 30, SMALL, BLACK, GREEN);
		sprintf(ciag, ">%1.1d  ", h);
		LCDPutStr(ciag, 60, 0, SMALL, BLACK, GREEN);
		if(s==60){m++;s=0;}
		if(m==60){h++;m=0;}
		
		if(LEFT_KEY_DOWN){LED_BCK_OFF;}//wylancza ekran
		if(RIGHT_KEY_DOWN){LED_BCK_ON;}//wlacza ekran
