	 ...
	 while( 1 )
		 {
			 ...
			 U=((U*3.3)/1023);
			 sprintf(s, "%f", U);
			 LCDPutStr(s, 60, 50, SMALL, WHITE, BLACK);
			 
			  WriteSpiCommand(VOLCTR);
        WriteSpiData(GetAdcChanel(ADC_CHN_7)/28);
			 
			 LCDSetCircle(65, 65, U*10, RED);
			 LCDClearScreen();
			 
			 BUZ_ON;
			 delay_ms((1/(U+0.1))/200); //Generuje dzwiek
			 BUZ_OFF;
			 delay_ms((1/(U+0.1))/200);
